...
(defruled |wordp replace-ones 0bar|
  (implies (wordp x '(#\0 #\1))
           (wordp (replace-ones x) '(#\0 #\|)))
  :enable replace-ones
  :induct (len x))
...
(defund step1 (inp n)
  (declare (xargs :guard (and (wordp inp'(#\0 #\1)) (natp n))))
...
(defruled |step1 applicable when #\1|
  (implies
   ...
   (equal
      ...
      (mks-> "1" "0|")))
  :enable (applicable step1 |wordp replace-ones 0bar|)
...
(defruled |step1 substp when #\1|
  ...
  :prep-lemmas (
    (defrule lemma
      ...
      :induct (len x)))
  :enable (step1 |wordp replace-ones 0bar|
     ...
     (defrule lemma3
       (implies
         ...
         (equal (list* #\0 #\| (replace-ones (nthcdr (+ 1 n) inp)))
                (replace-ones (nthcdr n inp))))
      :enable (|nthcdr as cons| replace-ones)))
  :enable (step2 |wordp replace-ones 0bar|)
